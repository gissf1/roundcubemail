 ...
 */
function rcube_webmail(){this.labels={};this.buttons={};this.buttons_sel={};this.gui_objects={};this.gui_containers={};this.commands={};this.command_handlers={};this.onloads=[];this.messages={};this.group2expand={};this.http_request_jobs={};this.menu_stack=[];this.dblclick_time=500;
this.message_time=/*5E3*/30000;
this.identifier_expr=/[^0-9a-z_-]/gi;this.env={request_timeout:180,draft_autosave:0,comm_path:"./",recipients_separator:",",recipients_delimiter:", ",popup_width:1150,popup_width_small:900};this.ref=
...
this.init_messageform()):"get"==this.env.action?(this.enable_command("download",!0),bw.mz&&"application/pdf"==this.env.mimetype?(a=0,$(this.gui_objects.messagepartframe).on("load",function(){if(a++)try{this.contentWindow.document,g.enable_command("print",!0)}catch(b){}})):this.enable_command("print",!0)):"print"==this.env.action&&this.env.uid&&this.print_dialog();this.gui_objects.mailboxlist&&(this.env.unread_counts={},this.gui_objects.folderlist=this.gui_objects.mailboxlist,this.http_request("getunread"));
...
g.enable_command("delete",!!a&&0>$.inArray(a,g.env.readonly_responses));a&&(b=g.get_frame_window(g.env.contentframe))&&(g.set_busy(!0),g.location_href({_action:"edit-response",_key:a,_framed:1},b))}).init().focus());break;case "login":d=$("#rcmloginuser"),d.bind("keyup",function(a){return g.login_user_keyup(a)}),""==d.val()?d.focus():$("#rcmloginpwd").focus(),window.jstz?(d=jstz.determine(),d.name()&&$("#rcmlogintz").val(d.name())):$("#rcmlogintz").val((new Date).getStdTimezoneOffset()/-60),$("form").submit(function(){$("input[type=submit]",
...
"dragover"==a.type)}).get(0).addEventListener("drop",function(a){return g.file_dropped(a)},!1));var f=function(a){return g.doc_mouse_up(a)};$(document.body).bind("mouseup",f).bind("keydown",function(a){return g.doc_keypress(a)});$("iframe").load(function(a){try{$(this.contentDocument||this.contentWindow).on("mouseup",f)}catch(b){}}).contents().on("mouseup",f);this.triggerEvent("init",{task:this.task,action:this.env.action});for(a in this.onloads)if("string"===typeof this.onloads[a])eval(this.onloads[a]);
else if("function"===typeof this.onloads[a])this.onloads[a]();this.start_refresh();this.start_keepalive()}else this.goto_url("error","_code=0x199")};this.log=function(a){window.console&&console.log&&console.log(a)};this.command=function(a,b,d,e){var f,h,m,k=!1;!d||!d.blur||e&&rcube_event.is_keyboard(e)||d.blur();if(this.busy&&("reset-search"!=a||"search"!=this.last_command)&&!a.match(/^menu-/))return!1;if(d&&d.href&&0>String(d.href).indexOf("#")&&rcube_event.get_modifier(e))return!0;if(!this.commands[a])return this.is_framed()&&
parent.rcmail.command(a,b),!1;if("mail"==this.task&&"compose"==this.env.action&&!this.env.server_error&&"save-pref"!=a&&0>$.inArray(a,this.env.compose_commands)){if(!this.env.is_sent&&this.cmp_hash!=this.compose_field_hash()&&!confirm(this.get_label("notsentwarning")))return!1;this.remove_compose_data(this.env.compose_id);this.compose_skip_unsavedcheck=!0}this.last_command=a;if("function"===typeof this.command_handlers[a])return f=this.command_handlers[a](b,d,e),void 0!==f?f:d?!1:!0;if("string"===
typeof this.command_handlers[a])return f=window[this.command_handlers[a]](b,d,e),void 0!==f?f:d?!1:!0;this.triggerEvent("actionbefore",{props:b,action:a,originalEvent:e});f=this.triggerEvent("before"+a,b||e);if(void 0!==f){if(!1===f)return!1;b=f}f=void 0;switch(a){case "login":this.gui_objects.loginform&&this.gui_objects.loginform.submit();break;case "logout":case "mail":case "addressbook":case "settings":this.switch_task(a);break;case "about":this.redirect("?_task=settings&_action=about",!1);break;
...
break;case "show":"mail"==this.task?(h=this.get_single_uid(),!h||this.env.uid&&h==this.env.uid||(this.env.mailbox==this.env.drafts_mailbox?this.open_compose_step({_draft_uid:h,_mbox:this.env.mailbox}):this.show_message(h))):"addressbook"==this.task&&(m=b?b:this.get_single_cid(),!m||"show"==this.env.action&&m==this.env.cid||this.load_contact(m,"show"));break;case "add":if("addressbook"==this.task)this.load_contact(0,"add");else if("settings"==this.task&&"responses"==this.env.action){if(e=this.get_frame_window(this.env.contentframe))this.set_busy(!0),
this.location_href({_action:"add-response",_framed:1},e)}else"settings"==this.task&&(this.identity_list.clear_selection(),this.load_identity(0,"add-identity"));break;case "edit":"addressbook"==this.task&&(m=this.get_single_cid())?this.load_contact(m,"edit"):"settings"==this.task&&b?this.load_identity(b,"edit-identity"):"mail"==this.task&&(h=this.get_single_uid())&&(e={_mbox:this.get_message_mailbox(h)},e[this.env.mailbox==this.env.drafts_mailbox&&"new"!=b?"_draft_uid":"_uid"]=h,this.open_compose_step(e));
break;case "save":var n;if(e=this.gui_objects.editform){if("search"!=this.env.action)if((n=$("input[name='_pagesize']",e))&&n.length&&isNaN(parseInt(n.val()))){alert(this.get_label("nopagesizewarning"));n.focus();break}else{if("reload"==b)e.action+="&_reload=1";else if("settings"==this.task&&0==this.env.identities_level%2&&(n=$("input[name='_email']",e))&&n.length&&!rcube_check_email(n.val())){alert(this.get_label("noemailwarning"));n.focus();break}$("input.placeholder").each(function(){this.value==
...
this.set_unread_count_display(a.id,!1))};this.doc_mouse_up=function(a){var b,d=rcube_event.get_target(a);if(!$(d).closest(".ui-dialog, .ui-widget-overlay").length){window.rcube_list_widget&&rcube_list_widget._instances.length&&$.each(rcube_list_widget._instances,function(b,d){d&&!rcube_mouse_is_over(a,d.list.parentNode)&&d.blur()});if(this.buttons_sel){for(b in this.buttons_sel)"function"!==typeof b&&this.button_out(this.buttons_sel[b],b);this.buttons_sel={}}setTimeout(function(a){var b,h,m,k,n=$(d).parents();
for(k=g.menu_stack.length-1;0<=k;k--)m=g.menu_stack[k],b=$("#"+m),!b.is(":visible")||d==b.data("opener")||d==b.get(0)||n.is(b.data("opener"))||m==h||"true"==b.attr("data-editable")&&$(d).parents("#"+m).length||"true"==b.attr("data-sticky")&&rcube_mouse_is_over(a,b.get(0))||g.hide_menu(m,a),h=b.data("parent")},10,a)}};this.doc_keypress=function(a){var b=function(a){var b,d=0>a?"prevAll":"nextAll",e=0>a?"last":"first";return g.focused_menu&&(b=$("#"+g.focused_menu))?(a=b.find(":focus").closest("li")[d](":has(:not([aria-disabled=true]))").find("a,input")[e](),
...
break;case 1:this.message_list.expand_all()}};this.init_threads=function(a,b){if(b&&b!=this.env.mailbox)return!1;for(var d=0,e=a.length;d<e;d++)this.add_tree_icons(a[d]);this.expand_threads()};this.add_tree_icons=function(a){var b,d,e,f,h=[],g=[],k,n=this.message_list.rows;for(k=a?n[a]?n[a].obj:null:this.message_list.tbody.firstChild;k;){if(1==k.nodeType&&(d=n[k.uid]))if(d.depth){for(b=h.length-1;0<=b&&!(e=h[b].length,e>d.depth?(f=e-d.depth,h[b][f]&2||(h[b][f]=h[b][f]?h[b][f]+2:2)):e==d.depth&&(h[b][0]&
2||(h[b][0]+=2)),d.depth>e);b--);h.push(Array(d.depth));h[h.length-1][0]=1;g.push(d.uid)}else{if(h.length){for(b in h)this.set_tree_icons(g[b],h[b]);h=[];g=[]}if(a&&k!=n[a].obj)break}k=k.nextSibling}if(h.length)for(b in h)this.set_tree_icons(g[b],h[b])};this.set_tree_icons=function(a,b){var d,e=[],f="",h=b.length;for(d=0;d<h;d++)2<b[d]?e.push({"class":"l3",width:15}):1<b[d]?e.push({"class":"l2",width:15}):0<b[d]?e.push({"class":"l1",width:15}):e.length&&!e[e.length-1]["class"]?e[e.length-1].width+=
...
this.selection_post_data();a._uid&&(this.show_contentframe(!1),this._with_selected_messages("delete",a))};this._with_selected_messages=function(a,b,d){var e=0,f="delete"==a||!this.is_multifolder_listing();if(this.message_list){var h,g,k,n=[],l=this.message_list.get_selection();h=0;for(len=l.length;h<len;h++)g=l[h],this.env.threading&&(e+=this.update_thread(g),k=this.message_list.find_root(g),k!=g&&0>$.inArray(k,n)&&n.push(k)),f&&this.message_list.remove_row(g,this.env.display_next&&h==l.length-1);
!this.env.display_next&&f&&this.message_list.clear_selection();h=0;for(len=n.length;h<len;h++)this.add_tree_icons(n[h])}0>e?b._count=-1*e:0<e&&f&&this.delete_excessive_thread_rows();f||(b._refresh=1);d||(d=this.display_message(this.get_label("move"==a?"movingmessage":"deletingmessage"),"loading"));this.http_post(a,b,d)};this.selection_post_data=function(a){"object"!=typeof a&&(a={});a._mbox=this.env.mailbox;if(!a._uid){var b=this.env.uid?[this.env.uid]:this.message_list.get_selection();a._uid=this.uids_to_list(b)}this.env.action&&
...
_flag:"delete"}),e=this.display_message(this.get_label("markingmessage"),"loading"),f=this.message_list,h=f?f.rows:{},g=0,k=0,n=a.length;k<n;k++)uid=a[k],h[uid]&&(h[uid].unread&&(b[b.length]=uid),this.env.skip_deleted?(g+=this.update_thread(uid),f.remove_row(uid,this.env.display_next&&k==f.selection.length-1)):this.set_message(uid,"deleted",!0));this.env.skip_deleted&&f&&(this.env.display_next&&f.rowcount||f.clear_selection(),0>g?d._count=-1*g:0<g&&this.delete_excessive_thread_rows());b.length&&(d._ruid=
this.uids_to_list(b));this.env.skip_deleted&&this.env.display_next&&this.env.next_uid&&(d._next_uid=this.env.next_uid);this.http_post("mark",d,e)};this.flag_deleted_as_read=function(a){var b,d,e,f=this.message_list?this.message_list.rows:{};"string"==typeof a&&(a=a.split(","));d=0;for(e=a.length;d<e;d++)b=a[d],f[b]&&this.set_message(b,"unread",!1)};this.uids_to_list=function(a){return this.select_all_mode?"*":1>=a.length?a.join(","):a};this.set_button_titles=function(){var a="deletemessage";this.env.flag_for_deletion||
!this.env.trash_mailbox||this.env.mailbox==this.env.trash_mailbox||this.env.delete_junk&&this.env.junk_mailbox&&this.env.mailbox==this.env.junk_mailbox||(a="movemessagetotrash");this.set_alttext("delete",a)};this.expunge_mailbox=function(a){var b,d={_mbox:a};a==this.env.mailbox&&(b=this.set_busy(!0,"loading"),d._reload=1,this.env.search_request&&(d._search=this.env.search_request));this.http_post("expunge",d,b)};this.purge_mailbox=function(a){var b,d={_mbox:a};if(!confirm(this.get_label("purgefolderconfirm")))return!1;
...
e,m._framed=1;b&&(m._gid=b);d&&(m._page=d);a&&(m._source=a);this.env.search_request&&(m._search=this.env.search_request);this.set_busy(!0,"loading");this.location_href(m,n)}};this.list_contacts_remote=function(a,b,d){this.list_contacts_clear();var e={},f=this.set_busy(!0,"loading");a&&(e._source=a);d&&(e._page=d);b&&(e._gid=b);this.env.source=a;this.env.group=b;this.env.search_request&&(e._search=this.env.search_request);this.http_request("mail"==this.env.task?"list-contacts":"list",e,f);"mail"!=
...
(h["image"==f.type?"src":"className"]=f[d]),"sel"==d&&(this.buttons_sel[b]=a))};this.set_pagetitle=function(a){a&&document.title&&(document.title=a)};
this.display_message=function(a,b,d,e){
	if(this.is_framed())return parent.rcmail.display_message(a,b,d);
	if(!this.gui_objects.message)return"loading"!=b&&(this.pending_message=[a,b,d,e]),1;
	b||(b="notice");
	e||(e=this.html_identifier(a));
	var f=b+(new Date).getTime();
	if(!d)switch(b){
		case "error":
		case "warning":
			d=2*this.message_time;
			break;
		case "uploading":
			d=0;
			break;
	...
	"loading"==b&&(e="loading",d=1E3*this.env.request_timeout,a||(a=this.get_label("loading")));
	if(this.messages[e])return this.messages[e].obj&&this.messages[e].obj.html(a),"loading"==b&&this.messages[e].labels.push({id:f,msg:a}),this.messages[e].elements.push(f),setTimeout(function(){g.hide_message(f,"loading"==b)},d),f;
	var h=$("<div>").addClass(b).html(a).data("key",e);
	$(this.gui_objects.message).append(h).show();
	this.messages[e]={obj:h,elements:[f]};
	"loading"==b?this.messages[e].labels=[{id:f,msg:a}]:"uploading"!=b&&h.click(function(){return g.hide_message(h)}).attr("role","alert");
	this.triggerEvent("message",{message:a,type:b,timeout:d,object:h});
	0<d&&setTimeout(function(){g.hide_message(f,"loading"!=b)},d);
	return f
};
this.hide_message=function(a,b){if(this.is_framed())return parent.rcmail.hide_message(a,b);if(this.gui_objects.message){var d,e,f,h,g=this.messages;if("object"===typeof a)h=$(a),d=h.data("key"),this.hide_message_object(h,b),g[d]&&delete g[d];
...
for(a in d)for(b in d[a].elements)d[a].obj&&this.hide_message_object(d[a].obj);this.messages={}};this.display_progress=function(a){if(a&&a.name){var b=this.messages["progress"+a.name];a.label||(a.label=this.get_label("uploadingmany"));b?!a.total||100<=a.percent?this.hide_message(b.obj):(a.text&&(a.label+=" "+a.text),b.obj.text(a.label)):(!a.percent||100>a.percent)&&this.display_message(a.label,"uploading",0,"progress"+a.name)}};
this.show_popup_dialog=function(a,b,d,e){
	if(this.is_framed())return parent.rcmail.show_popup_dialog(a,b,d,e);
	var f=$('<div class="popup">');
	"object"==typeof a?f.append(a):f.html(a);
	e=$.extend({
		title:b,buttons:d,modal:!0,resizable:!0,
width:500,
		close:function(a,b){$(this).remove()}
	},e||{});
	f.dialog(e);
	b=$(window);
	a=b.width();
	b=b.height();
	var h=f.width(),g=f.height();
	f.dialog("option",{
		height:Math.min(b-40,g+75+(d?50:0)),
		width:Math.min(a-20,h+36)}
	);
	$.each(e.button_classes||[],function(a,b){b&&$($(".ui-dialog-buttonpane button.ui-button",f.parent()).get(a)).addClass(b)});
	return f
...
this.http_error=function(a,b,d,e,f){
	d=a.statusText;
	if(a.status==200){
		d+=": "+a.responseText;
	};
	...
	a.abort();
	this.unload||(a.status&&d?this.display_message(this.get_label("servererror")+" ("+d+")","error")
		:"timeout"==b?this.display_message(this.get_label("requesttimedout"),"error")
		:0==a.status&&"abort"!=b&&this.display_message(this.get_label("connerror"),"error"),
			(b=a.getResponseHeader("Location"))&&"compose"!=this.env.action&&this.redirect(b),
			403==a.status?(this.is_framed()?parent:window).location.reload()
			:"keep-alive"==f&&setTimeout(function(){g.keep_alive();g.start_keepalive()},3E4)
	)
};
this.session_error=function(a){this.env.server_error=401;"compose"==this.env.action?(this.save_compose_form_local(),this.compose_skip_unsavedcheck=!0):a&&setTimeout(function(){g.redirect(a,!0)},2E3)};this.iframe_loaded=function(a){this.set_busy(!1,null,a);this.submit_timer&&clearTimeout(this.submit_timer)};this.multi_thread_http_request=function(a){var b,d,e=(new Date).getTime(),f=a.threads||1;a.reqid=e;a.running=0;a.requests=[];a.result=[];a._items=$.extend([],a.items);
...
e=/Adobe Reader|PDF|Acrobat/i;if(a&&a.enabledPlugin)return 1;if("ActiveXObject"in window){try{if(a=new ActiveXObject("AcroPDF.PDF"))return 1}catch(f){}try{if(a=new ActiveXObject("PDF.PdfCtrl"))return 1}catch(h){}}for(i=0;i<d;i++)if(a=b[i],"String"===typeof a){if(e.test(a))return 1}else if(a.name&&e.test(a.name))return 1;window.setTimeout(function(){$("<object>").css({position:"absolute",left:"-10000px"}).attr({data:g.assets_path("program/resources/dummy.pdf"),width:1,height:1,type:"application/pdf"}).load(function(){g.env.browser_capabilities.pdf=
...
b,d){setCookie(a,b,d,this.env.cookie_path,this.env.cookie_domain,this.env.cookie_secure)};this.get_local_storage_prefix=function(){this.local_storage_prefix||(this.local_storage_prefix="roundcube."+(this.env.user_id||"anonymous")+".");return this.local_storage_prefix};this.local_storage_get_item=function(a,b,d){var e,f;try{e=localStorage.getItem(this.get_local_storage_prefix()+a),f=JSON.parse(e)}catch(g){}return f||b||null};this.local_storage_set_item=function(a,b,d){try{return localStorage.setItem(this.get_local_storage_prefix()+
a,JSON.stringify(b)),!0}catch(e){return!1}};this.local_storage_remove_item=function(a){try{return localStorage.removeItem(this.get_local_storage_prefix()+a),!0}catch(b){return!1}};this.print_dialog=function(){bw.safari?setTimeout("window.print()",10):window.print()}}rcube_webmail.long_subject_title=function(g,u){if(!g.title){var a=$(g);a.width()+15*(u||0)>a.parent().width()&&(g.title=rcube_webmail.subject_text(g))}};
